...
> [!NOTE]
> This repository covers the standalone product code `instagram_email` only. Other product codes are outside its scope.

## Product facts

| API fact | Value |
|---|---|
| Product code | `instagram_email` |
...
1. Submit the input file to `POST /v1/tasks` with `task_type=instagram_email`.
...
Only `instagram_email`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/instagram-email-checker
- **Documentation:** https://docs.numberchecker.ai/instagram-email-checker
